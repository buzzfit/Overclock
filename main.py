...
# -*- coding: utf-8 -*-
"""
OVERCLOCK — a neon cyberpunk circuit shooter in one Python file (pygame-ce/pygbag friendly)

Notes for web build:
  • The main loop is now async (pygbag requirement) and yields with await asyncio.sleep(0).
  • Light audio "unlock": if the browser blocks audio init until a user gesture, the first
    keypress/click will retry mixer init.
  • Font fallback keeps text drawing even if 'Arial' isn't present in the web runtime.
...
Controls:
  ...
  • Quit: ESC (desktop only)

Dependencies: pygame-ce (for web build via pygbag)  •  desktop ok with pygame-ce or pygame
"""
...
def load_font(size, bold=False):
    # SysFont should fall back automatically, but add a guard for web runtimes
    try:
...
class SFX:
    def __init__(self):
        self.enabled = False
        self._sounds = {}
        # Try to init mixer (browser may block until user gesture)
        try:
    ...
    def _build_sounds(self):
        try:
            self._sounds = {
                "menu":      make_dual_tone(420, 840, 0.12, 0.45),
                "fire":      make_tone(980, 0.05, 0.5),
                "enemyfire": make_tone(360, 0.06, 0.45),
                "cancel":    make_dual_tone(700, 1200, 0.06, 0.5),
                "hit":       make_tone(1150, 0.05, 0.55),
                "hurt":      make_tone(160, 0.16, 0.5),
                "explode":   make_dual_tone(260, 180, 0.12, 0.48),
                "over_on":   make_dual_tone(900, 1200, 0.18, 0.5),
                "over_off":  make_dual_tone(400, 220, 0.15, 0.45),
                "win":       make_dual_tone(760, 1010, 0.2, 0.48),
                "lose":      make_tone(120, 0.35, 0.45),
                "natural":   make_tone(520, 0.05, 0.4),
                "clone":     make_dual_tone(660, 990, 0.08, 0.5),
            }
        except Exception:
            self.enabled = False
            self._sounds = {}
    ...
    def unlock(self):
        """Retry mixer init after a user gesture (needed in some browsers)."""
        if self.enabled:
            return
        try:
            pygame.mixer.init()
            ...
            self.enabled = False
        if self.enabled and not self._sounds:
            self._build_sounds()
...
class Game:
    def __init__(self):
        ...
        self.bigfont = load_font(48, bold=True)
    ...
    def handle_events(self):
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN:
                # Attempt to unlock audio on first user gesture (web browsers)
                self.sfx.unlock()
            ...
            if e.type == pygame.MOUSEBUTTONDOWN:
                # Attempt to unlock audio on first click (web)
                self.sfx.unlock()
...
# ---------- Async Entry (pygbag/browser & desktop) ----------
async def main():
    game = Game()
    while True:
        ...
        if game.state != "paused":
            game.update(dt)
        game.draw()
        # yield to the browser event loop (pygbag guidance)
        await asyncio.sleep(0)

if __name__ == "__main__":
    try:
        # Tip (desktop sim of web loop): you can try:
